...
sub html_convert_body {
	my ( $textwindow, $headertext, $cssblockmarkup, $poetrynumbers, $autotoc ) = @_;
...
# the header.
			...
			if ($autotoc) {
				...
				$completeheader = $selection;
				$h2 = "<h2 class=\"nobreak\" id=\"" . $aname . "\">";
			}

			# insert chapter heading unless already a para or heading open
			if ( not $selection =~ /<[ph]/ ) {
				$textwindow->ntinsert( "$step.0", $h2 );
				...
				while (1) {
					...
					if ( length($restofheader) and $autotoc) {
...
"\n\n<!-- Autogenerated TOC. Modify or delete as required. -->\n@contents\n<!-- End Autogenerated TOC. -->\n\n";
	...
	$textwindow->insert( $incontents, $contentstext ) if @contents and $autotoc;
	return;
}
...
sub htmlgenpopup {
	...
	if ( defined( $::lglobal{htmlgenpop} ) ) {
		...
		my $toc = $f0->Checkbutton(
			-variable    => \$::lglobal{autotoc},
			-selectcolor => $::lglobal{checkcolor},
			-text        => 'Generate TOC',
			-anchor      => 'w',
		  )->grid(
