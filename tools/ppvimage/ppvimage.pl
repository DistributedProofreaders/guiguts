...
if ($#ARGV >= 0) { # skip the gui
  if ($ARGV[0] eq '-h') { # print help
    ...
    print "The optional -gg argument prepends GuiGuts-style source line numbers to every line of output\n";
    print "The optional -o argument allows you to specify the name of the output file\n";
    }
  else { # process arguments
    $srctext = $ARGV[$#ARGV]; pop(@ARGV);
    while ($#ARGV >= 0) {
      $option = shift(@ARGV);
...
my $label1 = $mw -> Label(-text=>"ppvimage.pl post processing verification image checker.",
  -background=>"#eeeeee", -font=>[-weight=>'bold', -size=>11]);
my $label3 = $mw -> Label(-textvariable=>\$outfilename, -relief => 'sunken',
  -background=>"#eeeeee", -font=>[-weight=>'bold', -size=>10]);

my $button1 = $mw -> Button(-text => "Load", -command =>\&loadFile,
  -font=>[-weight=>'bold', -size=>10], -width=>10);
...
my $changeoutput = $mw -> Button(-text => "Change output log", -command => \&setoutputfile,
  -font=>[-weight=>'bold', -size=>9]);
$changeoutput -> grid(-row=>4, -column=>0, -sticky=>"nsw");
my $verbose = $mw -> Button(-text => "Verbose", -command => \&setverboseoutput,
  -font=>[-weight=>'bold', -size=>9], -width=>10);
my $notverbose = $mw -> Button(-text => "Terse", -command => \&setterseoutput,
  -font=>[-weight=>'bold', -size=>9], -width=>10);
...
my $about = $mw -> Button(-text => "About", -command =>\&about_ppv,
  -font=>[-weight=>'bold', -size=>9]);
my $addCSV = $mw -> Button(-text => "Include CSV output", -command =>\&setCSVoutput,
  -font=>[-weight=>'bold', -size=>9]);
my $removeCSV = $mw -> Button(-text => "Remove CSV output", -command =>\&noCSVoutput,
  -font=>[-weight=>'bold', -size=>9]);
...
sub setoutputfile {
  $outfile = $mw->getSaveFile(-defaultextension => ".log",
		      -filetypes        =>
		      [['Text Files',       ['.log', '.err']],
		       ['All Files',        '*',             ],
		      ],
...
sub loadFile {
  ...
  if ($srctext) {
    $txt -> insert('end',
     ...
     "You have selected:\n    ".$srctext."\n");
    if (not $makeCSV) {
      $txt -> insert('end',"Click the button below to also generate a comma separated (CSV) summary.\n");
      }
...
sub runProgram {
  ...
  if ($makeCSV ) {
    ...
    print CSVOUT ("filename, coded_wd, coded_ht, css_wd, css_ht, natural_wd, natural_ht, size_kb");
    }
  ...
  if (not ($gg)) {
    ...
    if ($txt) {
      ...
      if ($makeCSV ) {
        $txt -> insert('end',"Comma Separated Value version saved in\n   ".$CSVfile."\n");
        }
  ...
  foreach $_ (@book) { # find <img> tags and filenames
    ...
    if ($img =~ m/<img/i ) { # start of <img> tag
      ...
      while ($img =~ m/>/) { # complete <img> tag present
        ...
        if (not ($img =~ m/\/>$/)) {logprint ("img tag not properly closed",$reportline,"INFO","--> $img\n  WARNING: ","");}
        ...
        if (not ($src =~ m/^images\//)) { # image not in images folder
          logprint ("  image file not in /images folder",$errline,"KEY","  WARNING:","");
          }
        else { # add image to list of used images
          if ($src =~ m/[A-Z]/) {
            logprint ("  $src name is not lowercase",$errline,"KEY","  WARNING:","");  
            }
          my $usedimg = $src; $usedimg =~ s/images\/(.*)/$1/; # NB case sensitive: "images" must be lowerecase
          push (@imagelist, $usedimg);
          if (($usedimg =~ m/cover/) or ($usedimg =~ m/title/)) { # potential epub cover page
            if ($imgcover eq "") { $imgcover = $src; $imgcoverline = $reportline;}
            }
          }
        my $idspec = $img; $idspec =~ s/^.*id *= *(['"].*$)/$1/i;
        my $idqt = substr($idspec,0,1);
        if (length($idqt)>0) { # check if this is a coverpage
          if ($idqt eq '\'') {$idspec =~ s/'([^']*)'.*$/$1/; }
          elsif ($idqt eq '"') {$idspec =~ s/"([^"]*)".*$/$1/; }
          if ($idspec eq "coverpage") { $imgcover = $src; $imgcoverline = $reportline;} # note id=coverpage has higher precedence
          }
        ...
        if (length($altqt)>0) {logprint("  alt=$altqt$alt$altqt",$errline,"INFO","","");}
        ...
        if (length($ttlqt)>0) {logprint("  title=$ttlqt$ttl$ttlqt",$errline,"INFO","","");}
        my $warn = "  WARNING";
        my $wdstyl = "X";
        my $htstyl = "X";
...
sub imgdimens {
  ...
  elsif ($src =~ /\.jpe?g$/) {($x,$y)= jpegsize(\*IMGFILE);
    if (($x==0) and ($y==0)) { # unable to determine dimensions
  ...
  else {logprint("  unknown image type: $src",$errline,"KEY","","");
    }
  close IMGFILE;
  ...
  if ($size > MAXKBLINKED) {
    ...
    else {printf LOGFILE "  Filesize: %3u KB\n", $size; }
    }
  if ($makeCSV) {
    ...
    if ($bgimg =~ m/background-image/i) { # input contains start of defn
      ...
      while ($bgimg =~ m/[;}]/) { # spec closed
        ...
        if ($src eq $bgimg) { # no url provided (should be "none" or "inherit")
          ...
          if ($makeCSV) {print CSVOUT ("\n$src,,,,, ");}
          if (not ($src =~ m/^images\//)) { # image not in images folder
            logprint ("  image file not in /images folder",$reportline,"KEY","  WARNING:","");
            }
          else { # add image to list of used images
            if ($src =~ m/[A-Z]/) {
              logprint ("  $src name is not lowercase",$errline,"KEY","  WARNING:","");  
              }
            my $usedimg = $src; $usedimg =~ s/images\/(.*)/$1/;
            push (@imagelist, $usedimg);
            }
          $imgfile = File::Spec->catfile(dirname($srctext),$src);
          open (IMGFILE, "<",$imgfile) || do {
  ...
  foreach $_ (@book) {
    ...
    if ($usedimg =~ m/href/i) { # found a link
      ...
      while ($usedimg =~ m/href *= *['"]?images\/[^'" ]*['"]?/i) { # /images link
        ...
        push (@imagelist, $usedimg);
        if ($usedimg =~ m/[A-Z]/) {
          logprint ("  $usedimg name is not lowercase",$reportline,"KEY","  WARNING:","");  
          }
...
sub checkepubcover {
  ...
  foreach $_ (@book) {
    ...
    if ($linkrel =~ m/<\/head>/i) { # no need to look further
      ...
      if ($linkrel =~ m/<link *rel *= *['"]?coverpage['"]? *href *= *['"]?images\/[^'" ]*['"]?/i) { # got one
        $imgcover = $linkrel;
        $imgcover =~ s/^.*?<link *rel *= *['"]?coverpage['"]? *href *= *['"]?(images\/[^'" ]*)['"]?.*$/$1/i;
