...
sub fnjoin {
    ...
    # Ensure relevant indexes are up-to-date in case user has edited file since First Pass
    footnoterefresh( $::lglobal{fnindex} );
    footnoterefresh( $::lglobal{fnindex} - 1 );
...
#
# Use footnote marks to refresh index values stored for given footnote
# Needed if user might have edited text causing index value to become stale
sub footnoterefresh {
    my $fnidx      = shift;
    my $textwindow = $::textwindow;
    $::lglobal{fnarray}->[$fnidx][0] = $textwindow->index( 'fns' . $fnidx );
    $::lglobal{fnarray}->[$fnidx][1] = $textwindow->index( 'fne' . $fnidx );
    $::lglobal{fnarray}->[$fnidx][2] = $textwindow->index( 'fna' . $fnidx );
}
